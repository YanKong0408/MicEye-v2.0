...
def pointToHeatmap(pointList, gaussianSize=99, normalize=True, heatmapShape=(800, 800),imagescale=1,mode="KDE",kernel_bandwidth=1):
    canvas = np.zeros(heatmapShape)
    if mode=="Gauss":
        for p in pointList:
            if p[0][1]*imagescale/p[1] < heatmapShape[0] and p[0][0]*imagescale/p[1] < heatmapShape[1]:
                if canvas[int(p[0][1]*imagescale/p[1])][int(p[0][0]*imagescale/p[1])].all()==0:
                    canvas[int(p[0][1]*imagescale/p[1])][int(p[0][0]*imagescale/p[1])] = p[1]
                else:
                    canvas[int(p[0][1]*imagescale/p[1])][int(p[0][0]*imagescale/p[1])]+=(p[1]/10)
        ...
        weights=[]
        for p in pointList:
            x.append(int(p[0][0] * imagescale / p[1]))
            y.append(int(p[0][1] * imagescale / p[1]))
            weights.append(p[1])
        
        positions = np.vstack([x, y])
        kernel = gaussian_kde(positions, bw_method=kernel_bandwidth, weights=weights)  # 使用weights参数传入权重
        density = kernel(positions)
        plen=len(pointList)
        for i in range(plen):
            if x[i] < heatmapShape[1] and y[i] < heatmapShape[0]:
                if canvas[y[i]][x[i]].any() == 0:
                    canvas[y[i]][x[i]] = density[i]
                else:
                    canvas[y[i]][x[i]] += (density[i] / 10)

        # return heatmapColormap(canvas, cv2.COLORMAP_JET)
        g = cv2.GaussianBlur(canvas, (gaussianSize, gaussianSize), 0, 0)
        if normalize:
            g = cv2.normalize(g, None, alpha=0, beta=1,
                            norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
